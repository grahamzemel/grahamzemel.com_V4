...
mat3 rotX(float angle) { // this makes a rotation matrix around the x axis for a specific angle
  float s = sin(angle);
  float c = cos(angle);
...
mat3 rotZ(float angle) { // this makes a rotation matrix around the z axis for a specific angle
  float s = sin(angle);
  float c = cos(angle);
...
vec3 a = vec3(0.1, 0.1, 0.12); // Dark grey base
